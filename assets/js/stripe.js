...
jQuery( function( $ ) {
	...
	try {
		var stripe = Stripe( wc_stripe_params.key, {stripeAccount:wc_stripe_params.account_id} );
	} catch( error ) {
		console.log( error );
		return;
	}
	...
	var wc_stripe_form = {
		...
		isStripeChosen: function() {
			return $( '#payment_method_stripe' ).is( ':checked' ) || ( $( '#payment_method_stripe' ).is( ':checked' ) && 'new' === $( 'input[name="wc-stripe-payment-token"]:checked' ).val() ) || ( $( '#payment_method_stripe_sepa' ).is( ':checked' ) && 'new' === $( 'input[name="wc-stripe-payment-token"]:checked' ).val() );
		},
