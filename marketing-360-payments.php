...
class Marketing_360_Payments
{
    ...
    public static function get_account_details()
    {
      ...
      //  echo '<pre>'; print_r(get_option('woocommerce_stripe_settings')); echo '</pre>'; die();
        $details = json_decode(get_option('woocommerce_stripe_settings')['account_details']);
        if(get_option("woocommerce_stripe_settings")['test_stripe_Account'] == 'yes') {
            $details->stripeAccountId = 'acct_1MMzeGCW13GRLBhD';
            $details->stripeKey = 'pk_live_51LPFBOCYe7Lg3GLtWM85yvC7DajKQE98X1wqAKkdMjYP4sbwoy6C62cm6EGS72QYuIuw3dh7zMh3QQRhriGqEDVe00xEY5LqVO';
            /*                                $stripe_settings = get_option('woocommerce_stripe_settings');
                                            $stripe_settings['payment_request_button_locations'] = ['product','cart','checkout'];
                                             // update_option('woocommerce_stripe_settings', $stripe_settings);
                                	echo '<pre>'; print_r(get_option("woocommerce_stripe_settings")); echo '</pre>';
             */
        }
    ...
    //TODO: Provisionally
    // Callback to add the Stripe details to the M360 Account details after clicking "Save Changes" in the Payment Gateway Settings Screen
    public static function print_stripe_details()
    {
    ...
    public static function register_stripe_applepay_domain($token, $client_id, $client_secret, $account)
    {
        $domain_name = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : str_replace(array( 'https://', 'http://' ), '', get_site_url()); // @codingStandardsIgnoreLine
      //  $token = self::id_secret_get_access_token($client_id, $client_secret);
        $headers = self::get_m360_payments_request_headers($token, $account);

        // $api_url = self::get_payments_url(). '/' . self::VER . '/apple_pay/domains';
        $api_url = 'https://payments.marketing360.com/v1/stripe/v1/apple_pay/domains';
    //    $headers['Stripe-Account'] = $domain_name;
		...
		echo '<pre>'; print_r(['subdomain'=>$domain_name, 'JWT token'=>$token, 'URL Endpoint'=>$api_url, 'Headers'=>$headers, 'body' => ['domain_name' => $domain_name], 'API Response'=> $response]); echo '</pre>'; die();

    }

    // Get the Stripe details for the M360 Account using Account Number, ID, and Secret
    public static function get_stripe_applepay_domains($client_id, $client_secret, $account)
    {
        $token = self::id_secret_get_access_token($client_id, $client_secret);
        ...
        if (is_wp_error($token)) {
            ...
            // https://payments.marketing360.com/v1/api/account
            $response = wp_remote_post(
                self::get_payments_url(). '/' . self::VER . '/apple_pay/domains',
                [
                    'method'      => 'GET',
