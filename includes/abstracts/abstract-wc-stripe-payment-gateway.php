...
abstract class WC_Stripe_Payment_Gateway extends WC_Payment_Gateway_CC {
	...
	public function payment_icons() {
		return apply_filters(
			...
			array(
				'visa'       => '<img src="' . WC_M360_PAYMENTS_PLUGIN_URL . '/assets/images/visa.svg" class="stripe-visa-icon stripe-icon" alt="Visa" />',
				'amex'       => '<img src="' . WC_M360_PAYMENTS_PLUGIN_URL . '/assets/images/amex.svg" class="stripe-amex-icon stripe-icon" alt="American Express" />',
				'mastercard' => '<img src="' . WC_M360_PAYMENTS_PLUGIN_URL . '/assets/images/mastercard.svg" class="stripe-mastercard-icon stripe-icon" alt="Mastercard" />',
				'discover'   => '<img src="' . WC_M360_PAYMENTS_PLUGIN_URL . '/assets/images/discover.svg" class="stripe-discover-icon stripe-icon" alt="Discover" />',
				'diners'     => '<img src="' . WC_M360_PAYMENTS_PLUGIN_URL . '/assets/images/diners.svg" class="stripe-diners-icon stripe-icon" alt="Diners" />',
				'jcb'        => '<img src="' . WC_M360_PAYMENTS_PLUGIN_URL . '/assets/images/jcb.svg" class="stripe-jcb-icon stripe-icon" alt="JCB" />',
			)
		);
	}
	...
	public function create_intent( $order, $prepared_source ) {
		...
		$order_id = $order->get_id();
		WC_Stripe_Logger::log( "Marketing 360® Payments PaymentIntent $intent->id initiated for order $order_id" );
	...
	public function confirm_intent( $intent, $order, $prepared_source ) {
		...
		if( isset( $confirmed_intent->statusCode ) && $confirmed_intent->statusCode >= 400 ) {
			$confirmed_intent->error = $confirmed_intent->message;
		}
		...
		if ( 'succeeded' === $confirmed_intent->status ) {
			WC_Stripe_Logger::log( "Marketing 360® Payments PaymentIntent $intent->id succeeded for order $order_id" );
		} elseif ( 'requires_action' === $confirmed_intent->status ) {
			WC_Stripe_Logger::log( "Marketing 360® Payments PaymentIntent $intent->id requires authentication for order $order_id" );
		}
